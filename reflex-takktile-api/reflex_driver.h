...
namespace reflex_driver {
  ...
  class ReflexDriver {
  public:
    ReflexDriver(const std::string &network_interface,
                const std::string &finger_file_address,
                const std::string &tactile_file_address,
                const std::string &motor_file_name);

    bool calibrated;
    reflex_hand::ReflexHand reflex_hand;
    reflex_hand::ReflexHand *rh;
    reflex_hand::HandInfo hand_info;
  ...
  private:
    const int TACTILE_BASE_IDX[NUM_FINGERS] = {0, 18, 9};                     // Constant
    ...
    const std::string yaml_dir = "yaml";
    ...
    int calc_contact(const reflex_hand::ReflexHandState * const state, int finger, int sensor);
    ...
    void populate_motor_state(const reflex_hand::ReflexHandState* const state);
  };
}
