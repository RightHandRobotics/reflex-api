...
ReflexDriver::ReflexDriver(const std::string &network_interface,
                           const std::string &finger_file_name,
                           const std::string &tactile_file_name,
                           const std::string &motor_file_name): reflex_hand(network_interface), tactile_offset_values(NUM_FINGERS, std::vector<int>(0)) {
  ...
  finger_file_address = yaml_dir + finger_file_name;
  tactile_file_address = yaml_dir + tactile_file_name;
  motor_file_address = yaml_dir + motor_file_name;
...
uint16_t ReflexDriver::pos_rad_to_raw(float rad_command, int motor_idx) {
  ...
  if (command > reflex_hand::ReflexHand::DYN_MIN_RAW_WRAPPED) {
    printf("Finger %d set out of range (%d), reset to %d", motor_idx + 1,
             (uint16_t) command, reflex_hand::ReflexHand::DYN_MIN_RAW);
...
void ReflexDriver::move_until_any_contact(const float *velocities) {
  ...
  while(!any_touching) {
    ...
    for (int i = 0; i < NUM_FINGERS; i++) {
      all_zero = true;
      for (int i = 0; i < NUM_FINGERS; i++) {
        all_zero = (adjusted_velocities[i] == 0) && all_zero;
      }
      if (all_zero) {
        return;
      }
...
void ReflexDriver::move_until_each_contact(const float *velocities) {
  ...
  while(!all_touching) {
    ...
    rh->setServoTargets(raw_targets);
    touching_array = each_finger_touching(new bool[NUM_FINGERS]);

    for (int i = 0; i < NUM_FINGERS; i++) {
      all_zero = true;
      for (int i = 0; i < NUM_FINGERS; i++) {
        all_zero = (modified_velocities[i] == 0) && all_zero;
      }
      if (all_zero) {
        break;
      }
...
int ReflexDriver::calc_contact(const reflex_hand::ReflexHandState * const state, int finger, int sensor) {
...
void ReflexDriver::populate_motor_state(const reflex_hand::ReflexHandState * const state) {
  char buffer[10];
  for (int i = 0; i < NUM_SERVOS; i++) {
